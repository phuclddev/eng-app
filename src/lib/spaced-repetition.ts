...
function closestSupportedInterval(value: number) {
  return REVIEW_INTERVALS.reduce((closest, current) => {
...
export function calculateReviewUpdate(input: SchedulerInput) {
  ...
  const speedMultiplier =
    input.responseMs < 7_000 ? 1.15 : input.responseMs > 18_000 ? 0.9 : 1;

  const baseInterval =
    REVIEW_INTERVALS[Math.min(input.reviewCount, REVIEW_INTERVALS.length - 1)] ??
    REVIEW_INTERVALS.at(-1)!;
